...
func cfgSystemSection(entityName string, systemWindowDays int) templ.Component {
	return templ.ComponentFunc(func(ctx context.Context, w io.Writer) error {
		_, err := fmt.Fprintf(w, `<div style="max-width:480px;display:flex;flex-direction:column;gap:28px">
<div>
<h2 style="font-size:15px;font-weight:700;color:var(--text);margin:0 0 6px">Entity name</h2>
<p style="font-size:13px;color:var(--text3);margin:0 0 12px">Display name for this entity.</p>
<div style="display:flex;align-items:center;gap:10px">
<input id="entity-name-input" type="text" value="%s" style="width:240px;background:var(--bg);border:1px solid var(--border);border-radius:5px;padding:6px 10px;font-size:13px;color:var(--text);font-family:inherit"/>
<button id="entity-name-save" style="background:var(--accent);border:none;border-radius:5px;padding:6px 16px;cursor:pointer;font-size:13px;font-weight:500;color:#fff;font-family:inherit">Save</button>
<span id="entity-name-msg" style="font-size:12px;color:var(--text3)"></span>
</div>
</div>
<div>
<h2 style="font-size:15px;font-weight:700;color:var(--text);margin:0 0 6px">System rate window</h2>
<p style="font-size:13px;color:var(--text3);margin:0 0 12px">Number of days used to compute the rolling-window rate for the Income and Spend system entries.</p>
<div style="display:flex;align-items:center;gap:10px">
